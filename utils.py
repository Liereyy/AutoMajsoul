...
idx2str = ['1m', '2m', '3m', '4m', '5m', '6m', '7m', '8m', '9m',
...
def tile_transform_addbit(raw, array):
    for tile in raw:
        if tile in m_dict:
            if tile[0] == '0':
                array[4] += 1
...
def tile_transform_subbit(raw, array):
    for tile in raw:
        if tile in m_dict:
            if tile[0] == '0':
                array[4] -= 1
...
def tile_transform_dora(raw, stack):
    for dora in raw:
        if dora in m_dict:
            if dora[0] == '0':
...
def dist_to_side(tile):
    ...
    if tile in m_dict or tile in p_dict or tile in s_dict:
        return min(int(tile[0]) - 1, 9 - int(tile[0]))
    return 0


def print_safe_tiles(msg, tiles):
    m = ''
    p = ''
    s = ''
    z = ''
